...
// 2026-09-02: the form's starting state, and specifically the one field in it
// that decides whether anything a person writes is ever readable.
//
// The owner reported the blog and workshop tabs as empty twice. Both times
// the content had saved correctly - as a **draft**, because "Publish straight
// away" started unticked and the public read endpoint returns published items
// only (`services/api/src/content-repository.ts`). So "Save" did exactly what
// it said and nothing anyone wanted: the post existed, and no reader could
// reach it.
//
...
// already correct and stayed untouched. Moved here on 2026-09-06 with the
// defaults themselves, when `AuthoringPanel.tsx` was split into a composer
// per page.
