...
* **`Testimonial (own)` and `Workshop` denied**, unlike the sub-clinician columns. Nothing in the request asks for them, and marketing surfaces are not helpdesk work; `Content item` is granted only because "edit/upload content to existing patients" cannot be done without it. (`Testimonial moderation`, the row this replaced, was denied to helpdesk for the same reason; the 2026-09-02 amendment below removed moderation altogether.)

**2026-09-02 — `Testimonial moderation` becomes `Testimonial (own)`, and it is the only row in this table a patient writes.** The owner: *"the testimonial page by default should be read only with all published testimonials by various patients. for patients, when logged in, should have option to upload maximum one testimonial with option to update it. otherwise, submit a testimonial shouldn't be available for public and all kinds of clinicians. Also, there is no concept of review a testimonial — it should go live as soon as patient submits it from his account."*

Three things change at once, and they are one decision rather than three:

* **The author is a patient, and only a patient.** Every clinician column — sub-clinician, helpdesk, and the principal who "can do anything" — is denied. That is not an oversight in a table whose 2026-08-31 amendment deliberately gave the principal every clinical cell: a testimonial is *a patient's own words about their care*, and a practice that can write, edit, or approve those words is not collecting testimonials. The principal's authority over the practice stops at the patient's own account of it. This is the one row where "the principal can do anything" is answered *no*, on purpose.
* **At most one, and it stays editable.** The row grants `C R U` on a single record, and the store enforces the cardinality rather than the handler checking for one: a testimonial's id is derived from its author, so a second submission is the same record written twice. "Maximum one with the option to update it" is then not a rule anyone has to remember — it is the only shape the data can take.
* **There is no review, so there is no moderation row.** `GET /testimonials/pending`, `POST /testimonials/{id}/publish` and `POST /testimonials/{id}/reject` are gone, along with the anonymous public submission form they existed to guard. A patient's submission is published on write.

`D` is new to this table, and it is not deletion — 00-conventions.md's no-delete rule still holds, and this is a status transition to `withdrawn`. It is here because publication rests on the author's consent and consent that cannot be withdrawn is not consent: a patient who can publish their words must be able to retract them. Nobody else holds it, for the same reason nobody else holds `U`.
...
**~~A principal's own booking is `scheduled` immediately.~~ Superseded 2026-09-02 — every booking waits, whoever made it.** The owner, on seeing their own booking land straight on a patient's profile: *"when I assign an appointment to a patient, it should be visible to patient dashboard to be approved by principal clinician before it appears to patient profile."*

The exemption rested on "the approver approving themselves is a step with no decision in it", and that was wrong about what the step is *for*. It is not the principal proving something to themselves — it is the gate that decides **when a booking becomes real to the patient**, and a booking typed in error is caught by the same review whoever typed it. There is now one status for every new booking and one route out of it, and nothing left in the handler that reads a role to decide.
