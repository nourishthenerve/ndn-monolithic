...
**Actioned since the findings above were written**, on the owner's go-ahead and each on its own branch: the flag source is fixed (§3a), Core Web Vitals is measured and passing (§7), the a11y gate is fixed, re-enabled and now blocking (§7, action items 2 and 3), and the log-retention leak is closed at its single root cause (§4). The gate verdict is unchanged: G1 cannot be declared met while the apex criterion is blocked. **All four engineering action items this review raised are now closed**, all on 2026-08-21; what remains open is owner action and the AWS support case.
...
| 1.6.1 G1 cutover | Apex serves new site; legacy Lambda gone | **PARTIAL — one half done, one half blocked** | **Legacy retired: yes, completely** — Lambda, Function URL, log group, 5 IAM roles, 6 policies all deleted 2026-08-15; `get-function` → `ResourceNotFoundException`; R-06 closed by removal, not containment. **Apex: not yet cut over, no longer blocked.** The alias holder was identified 2026-08-21 via the AWS Support reply and verified directly: the `ndn-frontend` Amplify app in our own `803129122420`, not a third account (`155257` is AWS's Amplify service account, misread as a third party). Release is self-service; what remains is an owner-chosen window for the ~10–20 min outage. See [g1-cutover.md](../runbooks/g1-cutover.md) step 4. |
...
- **Repeat finding, worsening — log retention.** G0 §4 found 2 orphaned CloudWatch log groups from destroyed ephemeral PR stacks with no retention policy. There are now **10** (`Pr23`, `Pr25`, `Pr26`×2, `Pr27`×2, `Pr28`, `Pr29`, `Pr30`, `Pr999`), plus the live stack's own two implicit groups (`NdnWebStack-CustomCDKBucketDeployment...`, `NdnWebStack-HealthFunction...`), all still `Retention: None`. The twelve explicitly-constructed `/ndn/*` groups correctly carry 14 days. Total stored: ~47 KB, so £0.00 today — but TASK 0.5.2's DoD ("no log group has infinite retention") is still breached, and the count now grows with every PR that runs a deploy. The fix is the same one G0 recommended and nobody picked up: apply the retention aspect to CDK's implicitly-created Lambda log groups too, and set `removalPolicy: DESTROY` so ephemeral stacks clean up after themselves.
  ...
  **Closed 2026-08-21** — the last of this review's four engineering action items. Full writeup in [log-retention-volume-control.md](../runbooks/log-retention-volume-control.md#follow-up-2026-08-21--the-implicit-log-group-leak-gate-g1-4); in short: there were **13** by the time it was fixed, not 10 — PR #47 added one in the four days between this review and the fix, which is exactly the growth this finding predicted. All 13 are the same function, `BucketDeployment`'s `Custom::CDKBucketDeployment` singleton, the one Lambda in the app without an explicit log group; in production it now takes one from `createLogGroup()` like every other. Ephemeral stacks import a shared out-of-band group instead of creating one, because the first live run showed that a stack-owned group loses a race with its own teardown — `cdk destroy` deletes it, the Delete-event logs flush a moment later, and CloudWatch recreates it uncapped (measured: the recreated group's creation timestamp was 9 seconds later than the events inside it). PR stacks now create no log group for it at all, so the account's group count no longer grows with PR volume. A second Aspect makes recurrence a build failure: any `CfnFunction` without `LoggingConfig.LogGroup` makes `cdk synth` exit 1 (verified against the CLI). The 13 existing groups, which no deploy can reach, were given the 14-day policy by hand — `describe-log-groups --query 'logGroups[?retentionInDays==null]'` now returns empty, so the DoD is true on the live account for the first time. Nothing was deleted: retention empties the orphaned shells on its own within a day. Two things were found while fixing it, both now handled — the log-volume alarm summed 7 of 12 groups, missing both public GET endpoints (now 10 of 13, the rest named explicitly as out-of-budget), and the reason it cannot simply name all of them is a hard AWS limit of 10 metrics per alarm, probed against the real API rather than assumed.
...
- `NdnWebStack`'s distribution is `Deployed`, carrying `next.nourishthenerve.com` as its only alias with the three-SAN certificate attached — the deliberate decoupling from TASK 1.6.1's fix, as at this gate pass. (Superseded 2026-08-21: apex/`www` now rejoin `domainNames` in code, to deploy at the cutover window.)
- Legacy apex: `302` → `www`, `www` `200`, still served by the Amplify-managed distribution in `803129122420`. Untouched, and stays that way until the cutover window.
...
- **`infra/src/web-stack.ts` + `log-retention.ts` + `config.ts` + `log-retention-volume-control.md`** (§4, action item 4) — the implicit-log-group leak fixed at source, guarded against recurrence, and the 13 pre-existing groups capped on the account. This report's §4 amended in the same change.
...
**Done, 2026-08-21**, on its own branch — the last outstanding item on this gate's own checklist ("elaboration of the next phase's stubs", `06-gate-checklists.md`). Fourteen tasks at full §12 detail in [05-execution-plan.md](05-execution-plan.md), plus a `Gate G2` line carrying the checklist's gate-specific addition. Four things in it are worth reading even if the task specs are not:

1. **The one pushback in [09-self-audit.md](09-self-audit.md) was decided, and the decision is now recorded there.** Its recommendation — pull M2.3 (RBAC spine + audit) forward "alongside M1.1" — had expired, because Phase 1 shipped first. The surviving half was taken: **the spine goes first inside Phase 2, ahead of authentication.** Milestones renumber against the brief (M2.1 spine + audit, M2.2 auth, M2.3 notifications), which moves R-09's chokepoint from 3.2.x to **2.1.2** in the risk register and LL-02's blocker from M2.2 to **M2.3** in the long-lead register and ADR-0008. Every piece of the spine is pure domain code that needs no Cognito, which is why the reorder is possible at all.
2. **A repeat of §3a's shape, found in the same place §3a was.** `services/api/src/audit.ts` has exactly one implementation of `AuditWriter` — `InMemoryAuditLog`, an array in a Lambda's memory — and its header says why: "no DynamoDB table exists yet." `NdnDataStack` shipped at TASK 1.3.1 and that sentence stopped being true. Every `create`, `update`, `publish`, `unpublish`, `reject`, `cancel` and `confirm` the platform has performed has been written to memory and discarded when the invocation ended. This is not new breakage and not urgent while every feature flag is off, but it is the second correctly-deferred seam nobody came back for, and it is the mechanism that makes an authorisation boundary reviewable at all. **TASK 2.1.3** fixes it, including the `where` (`sourceIp`, `requestId`) that `04-data-model-rbac.md` asks for and the `AuditEvent` type never had.
3. **ADR-0004 could not survive elaboration and has been amended** (at this gate, ahead of Phase 2 — the same precedent as ADR-0017 at G0). D-09 asks for TOTP for clinicians and email-OTP for patients; Cognito's MFA policy is pool-wide and cannot be both. The decision is **two user pools**, which also makes the patient/clinician boundary structural rather than a group-membership check. It costs a Lambda authorizer, because API Gateway's built-in JWT authorizer binds one issuer — named in the ADR rather than discovered in Phase 2.
4. **A traceability gap this review did not catch.** §2 above reports coverage against FR IDs, and the repo cannot actually verify those claims: [07-traceability.md](07-traceability.md) holds a summary that points at itself for the matrix, and the source requirements brief it summarises (`ndn-planning-brief.md`, named in [00-index.md](00-index.md)) was never committed. Only `FR-DP-02`, `FR-DP-05` and the `FR-X-*`/`FR-WEB-*` IDs Phase 1 established are pinned to a requirement anywhere in the tree. Phase 2's task specs were written to cite only what is pinned, plus brief section references. Nothing is blocked, but "every FR maps to ≥1 task and ≥1 test" is currently an unverifiable claim — see the new engineering action item below.
...
4. ~~**Fix the log-group retention leak** (§4) — repeat finding, five times larger than at G0, and it grows per PR~~ — **done 2026-08-21**, and it had grown once more (13, not 10) in the four days it stayed open. One root cause for all of them: `BucketDeployment`'s own Lambda, the only function in the app without an explicit log group. Fixed at the source, guarded by a synth-time error so it cannot recur, and the pre-existing groups capped by hand — the account now has zero log groups with infinite retention. See §4 and [log-retention-volume-control.md](../runbooks/log-retention-volume-control.md#follow-up-2026-08-21--the-implicit-log-group-leak-gate-g1-4). **All four engineering action items from this review are now closed.**

**Raised after the review, during Phase 2 elaboration:**

5. **Reconstruct the FR/NFR traceability matrix into [07-traceability.md](07-traceability.md).** The file currently holds a coverage summary that cites itself as the location of the matrix, and the requirements brief it summarises is not in the repo — so the coverage numbers in §2 and in [09-self-audit.md](09-self-audit.md) cannot be checked by anyone reading this tree. Close it before Gate G2, whose checklist audits requirement traceability. Needs the source brief from the owner, or a matrix rebuilt from the task specs themselves and marked as such.
