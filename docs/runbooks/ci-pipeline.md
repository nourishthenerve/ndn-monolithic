...
**Writing test fixtures that will not trip it (2026-09-01).** The scan reads **full git history**, and there is no allowlist file — deliberately, since an allowlist is a place for real leaks to be parked. Two consequences worth knowing before you write a fixture:

- **A fake credential that *looks* like a credential still fails the build.** A hand-written JWT (`header.<44 chars of base64>.signature`) assigned to a constant named `TOKEN` is exactly the shape gitleaks' `generic-api-key` rule exists to catch, and it caught one in `PatientNotifications.render.test.tsx`. The rule was right: the string was invented, but nothing about it said so. Prefer an obviously-fake opaque value (`'test-access-token'`), or build the token at runtime from its parts when a test genuinely needs a decodable one — `AssessmentForm.render.test.tsx`'s `tokenFor()` does the latter and passes.
- **Fixing the file is not enough once it is committed.** History is what is scanned, so the offending commit has to be rewritten (amend or rebase) and force-pushed, not corrected in a follow-up commit. Verify locally with the exact CI command before pushing — and note that a stale `refs/remotes/origin/…` or a leftover backup branch keeps the old commit reachable, so a local run can keep failing after a correct rewrite until those refs are updated or dropped:

```sh
gitleaks detect --source . --redact -v      # same flags as the workflow
git for-each-ref --contains <old-sha>       # what is still keeping it alive
```
