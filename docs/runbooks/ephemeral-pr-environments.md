...
- **`prLabel: string`** (e.g. `pr-999`) — mixed into both explicit CloudWatch log group names (`/ndn/pr-999/health-function`, `/ndn/pr-999/smoke-test-function` vs. production's fixed `/ndn/health-function`). CloudWatch Logs group names are a flat per-account/region namespace, not scoped by CloudFormation stack — without this, two PR stacks deployed concurrently (two open PRs; different refs aren't cancelled by the workflow's own `concurrency` group) would collide on the second one's `CreateLogGroup` call.
