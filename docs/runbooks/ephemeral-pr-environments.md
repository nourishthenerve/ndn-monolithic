...
- **`prLabel: string`** (e.g. `pr-999`) — mixed into every explicit CloudWatch log group name the stack creates (`/ndn/pr-999/health-function`, `/ndn/pr-999/smoke-test-function`). The one Lambda this does **not** cover is CDK's `Custom::CDKBucketDeployment`, which since 2026-08-21 writes to a shared, out-of-band `/ndn/pr-env/site-deployment` that no stack creates or deletes: it is the one function `cdk destroy` invokes as part of its own teardown, so a stack-owned group loses a race with its deletion and CloudWatch silently recreates it uncapped. See [log-retention-volume-control.md](log-retention-volume-control.md#the-first-live-run-found-a-second-race--and-it-is-why-ephemeral-stacks-import). CloudWatch Logs group names are a flat per-account/region namespace, not scoped by CloudFormation stack — without this, two PR stacks deployed concurrently (two open PRs; different refs aren't cancelled by the workflow's own `concurrency` group) would collide on the second one's `CreateLogGroup` call.
