...
| `patients.administration.enabled` | `POST /patients`, `POST /patients/{id}/reset-password` — staff-mediated account creation and password reset (D-29, 2026-08-29). Stays off until TASK 2.5.1 can approve someone. Replaces `auth.patientRegistration.enabled`, which gated TASK 2.2.3's now-retired self-registration route ([patient-account-provisioning.md](patient-account-provisioning.md)) |
