...
7. `contentAssignment.enabled` (3.5.1) — needs Track A's `content.authoring.enabled` already on with real published content to assign, not just the flag.
8. `messaging.enabled` (3.6.1) — patient↔clinician messaging.
9. `video.signalling.enabled` (4.1.1), then `video.callAuthz.enabled` (4.2.1) — a socket that connects but cannot join is the deliberately safe intermediate state TASK 4.2.1's own comment names; both are otherwise independent flags.
10. `video.turn.enabled` (4.4.1) — **its own separate, non-LL blocker, open since Gate G4, closed 2026-08-30:** the owner provisioned a real Cloudflare Realtime/Calls TURN key (`CLOUDFLARE_TURN_KEY_ID` in `infra/src/config.ts`, and `/ndn/cloudflare-turn-api-token` in SSM, both real — `video-calls.md`). This flag still sits in Track B, gated by LL-05/LL-06 like every flag above it — the TURN key alone doesn't move it ahead of that gate, it only means the flag will do something real once Track B's own gate clears, rather than silently degrading every call to `call-failed`.
...
- ~~Flip `video.turn.enabled` expecting it to work before a real Cloudflare TURN key is provisioned — it will not fail loudly, it will silently degrade every fallback to `call-failed`.~~ **Moot, 2026-08-30** — a real key is provisioned; this flag's remaining gate is Track B's own LL-05/LL-06, same as every flag above it.
