...
- **Blast radius:** the legacy static site is **not** served from this account. `list-buckets` confirms the `nourishthenerve` bucket holds only `clients/` and `posts/` — no HTML, no site assets — so the brochure site's origin is elsewhere. (Identified 2026-08-21: the `ndn-frontend` Amplify app in this same account, `eu-west-2`, whose Amplify-managed distribution is `d2z3fclxq13w3z.cloudfront.net` — not a third account as assumed here. See [g1-cutover.md](g1-cutover.md). The blast-radius conclusion is unaffected: nothing deleted here served the site.) Verified after deletion: apex `302` → `www`, `www` `200`, still serving.
