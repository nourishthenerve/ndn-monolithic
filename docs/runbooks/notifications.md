...
- ~~**The SMS provider itself.** `sms.ts`'s guard chain still calls no provider — `Notifier`'s SMS attempt runs the same chain 0.5.3 built and proved, and TASK 2.3.2 is what makes its `Sent` branch real.~~ **Built at TASK 2.3.2**, the same day — see [`sms-provider.md`](sms-provider.md). `SendSmsParams` gained a `body` field for the rendered SMS text as part of that task; `notifications.ts`'s call site here was updated to pass it.
