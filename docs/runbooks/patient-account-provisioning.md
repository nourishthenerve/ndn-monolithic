...
- **No staff-facing UI.** Today, creating an account or resetting a password means a principal clinician (or whoever holds a principal credential) calling `POST /patients`/`POST /patients/{id}/reset-password` directly — `curl`, Postman, or an equivalent — with a bearer token from a real sign-in. A small internal form is the obvious next increment once the API is proven live, but building one before the mechanism itself was verified would have been guessing at a UI for an untested backend.
