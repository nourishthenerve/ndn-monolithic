...
- **Not yet run**, and named rather than reported as done: the plan's browser check (a full sign-in leaves no token in `localStorage`/`sessionStorage`, checked in the browser rather than inferred; a captured cookie stops refreshing after sign-out) needs the flag on and a deployed pool domain. D-29 (2026-08-29) removes the SES/mail blocker this line originally named for patients — a patient's password is created and relayed over WhatsApp, never emailed (`docs/runbooks/patient-account-provisioning.md`). **Correction, 2026-08-29:** this line previously claimed SES production access "still gates" a clinician sign-in check the way it once gated a patient one. Checked live against the deployed pool (`describe-user-pool` → `EmailConfiguration.EmailSendingAccount: COGNITO_DEFAULT`) — the clinician pool has never been wired to this account's own SES identity at all; its invite email has always used Cognito's own built-in sender, unaffected by LL-01's denial either way. That claim was simply wrong, not stale-but-once-true. It is superseded regardless: D-30 (`docs/runbooks/clinician-accounts.md`) removes Cognito's invite email from clinician onboarding entirely, the same "no email in the loop" pivot D-29 already made for patients.
