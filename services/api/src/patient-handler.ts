...
// `createPatientProfileStore` (dynamo-store.ts) rather than a bespoke
// store — `patient-admin-handler.ts` and every other handler that needs a
// `PatientRepository` wire the identical `PAT#<id>` / `PROFILE` key shape
// through that same factory, which is also what keeps GSI3's directory
// projection on the row after a `PATCH`.

import { systemClock } from './clock.js';
import { DynamoAuditLog } from './dynamo-audit-log.js';
import { createPatientProfileStore, DynamoAssignmentStore } from './dynamo-store.js';
