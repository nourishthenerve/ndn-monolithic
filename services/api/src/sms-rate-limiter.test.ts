...
describe('InMemoryRateLimiter', () => {
  it('allows up to the limit within a window, then blocks', async () => {
    ...
    expect(await limiter.tryConsume('patient-1')).toBe(true);
    expect(await limiter.tryConsume('patient-1')).toBe(true);
    expect(await limiter.tryConsume('patient-1')).toBe(true);
    expect(await limiter.tryConsume('patient-1')).toBe(false);
  });
  ...
  it('tracks each principal independently', async () => {
    ...
    expect(await limiter.tryConsume('patient-1')).toBe(true);
    expect(await limiter.tryConsume('patient-1')).toBe(false);
    expect(await limiter.tryConsume('patient-2')).toBe(true);
  });
