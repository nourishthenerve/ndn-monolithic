...
//
// `testMatch` is scoped to exactly the two files this task adds:
// vitest.pr-env.config.ts's `pr-env/**/*.test.ts` glob is broader and
// already owns contract.test.ts/integration.test.ts, which import
// `vitest`'s describe/it/expect, not Playwright's — letting this config
// pick those up too would fail immediately on the wrong test API.
export default defineConfig({
  testDir: './pr-env',
  testMatch: ['a11y-full.test.ts', 'keyboard.test.ts'],
  timeout: 30_000,
  fullyParallel: true,
